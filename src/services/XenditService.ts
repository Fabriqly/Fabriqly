...
export class XenditService {
  ...
  async refundInvoice(
    ...
    try {
      console.log('Refunding invoice:', { invoiceId, amount, reason });

      if (!Invoice) {
        throw AppError.badRequest('Invoice service is not available in the Xendit SDK.');
      }

      // Xendit Invoice refund API
      // Note: Check Xendit documentation for exact API structure
      ...
      // The exact method name may vary - check Xendit SDK documentation
      const refund = await (Invoice as any).expireInvoice({
        id: invoiceId,
        // Note: Xendit may use different methods for refunds
        // This is a placeholder - actual implementation depends on Xendit API
      });

      // For now, return a mock response structure
      // TODO: Update with actual Xendit refund response structure
      return {
        id: refund.id || `refund-${Date.now()}`,
        invoiceId: invoiceId,
        amount: amount || 0,
        status: 'PENDING', // or 'COMPLETED' based on Xendit response
        reason: reason,
      };
    } catch (error: any) {
      console.error('Xendit invoice refund failed:', error);
      console.error('Error details:', {
        message: error.message,
        code: error.code,
        status: error.status
  ...
  /**
   * Void a disbursement (if not yet processed)
   */
  async voidDisbursement(disbursementId: string, reason: string): Promise<void> {
    try {
      console.log('Voiding disbursement:', { disbursementId, reason });

      if (!Payout) {
        throw AppError.badRequest('Payout service is not available in the Xendit SDK.');
      }

      // Get disbursement first to check status
      const disbursement = await this.getDisbursement(disbursementId);

      // Check if disbursement can be voided (must be pending)
      if (disbursement.status !== 'PENDING' && disbursement.status !== 'CREATED') {
        throw AppError.badRequest(
          `Cannot void disbursement. Current status: ${disbursement.status}. Only pending disbursements can be voided.`
        );
      }

      // Xendit may not have a direct void API
      // In that case, we'll just mark it as cancelled in our system
      // and prevent the payout from being processed
      // TODO: Check Xendit API documentation for actual void/cancel method

      console.log('Disbursement voided successfully (marked as cancelled in system)');
    } catch (error: any) {
      console.error('Xendit disbursement void failed:', error);
      throw AppError.badRequest(
        'Disbursement void failed',
        error.message || 'Unknown error occurred'
      );
    }
  }
