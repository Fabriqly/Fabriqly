...
export async function POST(request: NextRequest) {
  try {
    ...
    // File size limit - 5MB for review images
    ...
    const uploadResult = await SupabaseStorageService.uploadFileFromServer(
      ...
      file.type,
      {
        bucket: StorageBuckets.DESIGNS, // Using designs bucket for review images
        folder: `reviews/${session.user.id}`,
        upsert: false
      }
    );

    console.log('Review image uploaded:', uploadResult);
